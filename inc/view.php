...
EOT;
	...
	function view_projects() {
		...
		foreach ($projects as $p) {
			...
			print "<tr><td>Total:</td><td>{$total} (with {$percent} % solved)</td></tr>";
			print "</table>";
		}
	}
	...
	function view_addproject() {
		...
		view_options("owner", $db->get_enabled_users(), $session->userid);
		print "</td></tr>\n";
...
?>	<td>Username:</td>
	...
	<td>Password:</td>
	<td><input type="password" name="pass1" size="20" maxlength="20" value="" /></td>
</tr><tr>
	<td>Password again:</td>
	<td><input type="password" name="pass2" size="20" maxlength="20" value="" /></td>
</tr>
<?php
	...
	function view_editproject() {
		...
		print "<tr><td>Name:</td><td>";
		view_input("name", $project->name);
		print "</td></tr>\n";
		
		print "<tr><td>Owner:</td><td>";
		view_options("owner", $db->get_enabled_users(), (int)$project->owner);
		print "</td></tr>\n";
...
?>
	<td>Username:</td>
	<td><input type="text" name="name" size="20" maxlength="20" value="<?php echo $user->name; ?>" /></td>
</tr><tr>
	<td>Password:</td>
	<td><input type="password" name="pass1" size="20" maxlength="20" value="" />
...
<?php
		print "<tr><td>Full name:</td><td>";
		view_input("fullname", $user->fullname);
		print "</td></tr>\n";
		
		print "<tr><td>Email address:</td><td>";
		view_input("email", $user->email);
		print "</td></tr>\n";
...
?>
	...
	<td>Old password:</td>
	<td><input type="password" name="opass" size="20" maxlength="20" value="" /></td>
</tr><tr>
<?php
		print '<tr><td /><td style="text-align: right;">'
			.'<input type="submit" value="Save" /> '
	...
	function view_list($listtype) {
		...
		switch ($listtype) {
			case 'my':
				$bugs = $db->users_bugs($session->userid, $session->project);
				view_title("My cases:");
				break;
			case 'open':
		...
		view_options("project", $projects, $session->project, 'onchange="this.form.submit();"');
...
EOT;
			foreach ($bugs as $b) {
				$class = preg_replace("/[^a-z]/", "", strtolower($b->status));
				$critical = (substr($b->priority,0,1) == '1') ? ' class="critical"' : '';
				if ($short) {
					if (strlen($b->name) > 40)
						$b->name = substr($b->name, 0, 38) . "...";
					print <<<EOT
	<tr class="$class">
		<td><img class="smallicon" src="{$b->icon}" alt="{$b->type}"/></td>
		<td><a href="{$config['path']}?view=bug&amp;bugid={$b->id}">#{$b->id}: {$b->name}</a></td>
...
EOT;
	...
	<tr>
		<td><div class="bug">
			<h4 class="$class"><a href="{$config['path']}?view=bug&amp;bugid={$b->id}">#{$b->id}: {$b->name}</a></h4>
			<p><img class="icon" src="{$b->icon}" alt="" />
				<b>{$b->type}</b> with priority <b$critical>{$b->priority}</b>,
				opened {$b->opened}.<br />
...
?></td></tr>
	<tr><td>Assigned to:</td>
		<td><?php
		view_options("assigned", $db->get_enabled_users(), $bug->assigned);
		?></td></tr>
	<tr><td>Category:</td>
		<td><?php
		view_options("category", $db->get_categories(), $bug->type);
		?></td></tr>
	<tr><td>Priority:</td>
		<td><?php
		view_options("priority", $db->get_priorities(), $bug->priority);
		?></td></tr>
	<tr><td>Version:</td>
		<td><?php
		view_input("version", $bug->version);
		?></td></tr>
	<tr><td>Computer:</td>
		<td><?php
		view_input("computer", $bug->computer);
		?></td></tr>
	<tr><td></td>
		<td><input type="checkbox" name="subscribe" <?php
		print ($subscription ? "checked=\"chechked\"" : "");
		?>>Send me email notifications.</input></td></tr>
	<tr><td>Log message:</td><td /></tr>
	<tr><td colspan="2"><?php
		view_textarea("message");
		?></td></tr>
	<tr><td>Status:</td>
		<td><?php
		view_options("status", $db->get_statuses(), $bug->status);
		?></td></tr>
...
<?php
	...
	function view_create() {
		...
		if (count($projects) == 0) {
			print '<p>Please ask your administrator to create at least one project first.</p>';
			return;
		}
...
<table>
	<tr><td>Title:</td><td><?php
		view_input("title");
		?></td></tr>
	<tr><td>Content:</td><td/></tr>
	<tr><td colspan="2"><?php
		view_textarea("content");
		?></td></tr>
	<tr><td /><td style="text-align:right">

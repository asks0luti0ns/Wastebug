...
EOT
		...
		function create_user($uname, $pass, $fname, $email, $super) {
...
VALUES ('$Puname', '$pass', '$Pfname', '$Pemail', '$Psuper');
EOT
);		
		}
...
EOT
                ...
                function set_password($uid, $pass) {
                        
                        pg_query($this->conn, <<<EOT
UPDATE wastebug.wb_users SET password = '$pass' WHERE id = '$uid';
EOT
);
                }
...
EOT
		...
		function name_project($id) {
			...
			if(pg_num_rows($result) != 1) return
				"Retirement-Planning for Kids - AS/400 version";
			$obj = pg_fetch_object($result, 0);
			return $obj->name;
		}
		...
		function create_case($name, $project, $assigned, 
...
EOT
		...
		function users_bugs($user, $project) {
			$array = $this->get_bugs(
				"open = true AND ownerid = '$user'", $project);
			return $array;
		}
